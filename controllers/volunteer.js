...
class authController {
  ...
  async changePassword(req, res) {
    try {
      let user = await Volunteer.findOne({ _id: req.volunteer._id })
      const passwordCheck = await bcrypt.compare(req.body.old, user.password);
      if (!passwordCheck) {
        res.status(400).json({ error: "Internal error" });
      }
      const salt = await bcrypt.genSalt(10);
      let password = await bcrypt.hash(req.body.new, salt);
      user.password = password;
      await user.save()
      res.status(200).json({ success: true })
    } catch (e) {
